    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not device:
                ...
                # Obter acesso root
                if client_id in [9, 10]:  # Entech DNS 1 e 2
                ...
                remote_file_entech = "/usr/local/etc/unbound/sitesblock.conf"
                tmp_remote_file = "/tmp/sitesblock.conf"

                # Diretório de download do sitesblock.conf para Entech DNS 1 e 2
                if client_id in [9, 10]:
                    remote_file_true = remote_file_entech
                else:
                ...
                if bloqueio_realizado and "sucesso" in bloqueio_realizado.lower():
                    ...
                    elif client_id in [9, 10]:  # Entech DNS 1 e 2
                        connection.send_command_timing(
                            "cp /usr/local/etc/unbound/sitesblock.conf /usr/local/etc/unbound/sitesblock.conf.old")
                        connection.send_command_timing(
                            "cp /tmp/sitesblock.conf /usr/local/etc/unbound/")
                        connection.send_command_timing(
